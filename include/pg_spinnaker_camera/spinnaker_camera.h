...
class SpinnakerCamera {
 public:
  explicit SpinnakerCamera(std::string serial_number): serial_(serial_number),
    acquiring_(false) {
    system_ = Spinnaker::System::GetInstance();
    cam_list_ = system_->GetCameras();
    unsigned int num_cameras = cam_list_.GetSize();
    ROS_INFO_STREAM("[pg_spinnaker_camera]: Number of cameras detected: " <<
      num_cameras);
    ...
    if (cam_ptr_) {
      ...
      ROS_ERROR_STREAM("[pg_spinnaker_camera]: FAILED to open camera " <<
        serial_number << ". Is it connected?");
    }
  }

  ~SpinnakerCamera() {
    ROS_INFO_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
      ") Destructor called.");
    if (cam_ptr_) end();
  ...
  void printDeviceInfo() {
    ...
    Spinnaker::GenApi::INodeMap &device_node_map =
      cam_ptr_->GetTLDeviceNodeMap();
    Spinnaker::GenApi::FeatureList_t features;
    Spinnaker::GenApi::CCategoryPtr category =
      device_node_map.GetNode("DeviceInformation");
    if (Spinnaker::GenApi::IsAvailable(category) &&
        Spinnaker::GenApi::IsReadable(category)) {
      category->GetFeatures(features);
      for (auto it = features.begin(); it != features.end(); ++it) {
        Spinnaker::GenApi::CNodePtr feature_node = *it;
        std::cout << feature_node->GetName() << " : ";
        Spinnaker::GenApi::CValuePtr value_ptr =
          (Spinnaker::GenApi::CValuePtr) feature_node;
        std::cout << (Spinnaker::GenApi::IsReadable(value_ptr) ?
          value_ptr->ToString() : "Node not readable");
        std::cout << std::endl;
      }
    } else {
    ROS_ERROR_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
  ...
  bool set(const std::string &property_name, const std::string &entry_name) {
    ...
    Spinnaker::GenApi::CEnumerationPtr enumerationPtr =
      node_map_->GetNode(property_name.c_str());
    if (!Spinnaker::GenApi::IsImplemented(enumerationPtr)) {
      ROS_ERROR_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
        ") Enumeration name " << property_name << " not implemented.");
      return false;
    }
      ...
      if (Spinnaker::GenApi::IsWritable(enumerationPtr)) {
        ...
        if (Spinnaker::GenApi::IsAvailable(enumEmtryPtr)) {
          if (Spinnaker::GenApi::IsReadable(enumEmtryPtr)) {
            ...
            ROS_WARN_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
  ...
  bool set(const std::string &property_name, const float &value) {
    ...
    if (Spinnaker::GenApi::IsAvailable(floatPtr)) {
      if (Spinnaker::GenApi::IsWritable(floatPtr)) {
        ...
        ROS_WARN_STREAM("[pg_spinnaker_camera]: (" << serial_ << ") Feature " <<
  ...
  void execute(const std::string& command) {
    Spinnaker::GenApi::CCommandPtr commandPtr =
      node_map_->GetNode(command.c_str());
    if (!Spinnaker::GenApi::IsAvailable(commandPtr) ||
      !Spinnaker::GenApi::IsWritable(commandPtr)) {
      ROS_ERROR_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
        ") unable to command " << command << ". Aborting...");
    } else {
      commandPtr->Execute();
    }
  }
  ...
  int getInteger(const std::string& property_name) {
    Spinnaker::GenApi::CIntegerPtr integerPtr =
    ...
    if (Spinnaker::GenApi::IsAvailable(integerPtr)) {
      if (Spinnaker::GenApi::IsReadable(integerPtr)) {
        ...
        ROS_WARN_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
          ") Feature " << property_name << " not readable.");
      }
    } else {
      ROS_WARN_STREAM("[pg_spinnaker_camera]: (" << serial_ <<
